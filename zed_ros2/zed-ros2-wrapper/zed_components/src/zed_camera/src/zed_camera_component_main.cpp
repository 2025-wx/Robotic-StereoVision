...
ZedCamera::ZedCamera(const rclcpp::NodeOptions &options)
  ...
  if ((ZED_SDK_MAJOR_VERSION * 10 + ZED_SDK_MINOR_VERSION) <
      (SDK_MAJOR_REQ * 10 + SDK_MINOR_REQ)) {
    RCLCPP_ERROR_STREAM(
        get_logger(),
...
void ZedCamera::init() {
  ...
  mDiagUpdater.add(mCameraName, this, &ZedCamera::callback_updateDiagnostic);
...
void ZedCamera::getGeneralParams() {
  ...
  if (mSvoMode && !mSvoRealtime) {
    RCLCPP_INFO(get_logger(),
                " * [SVO mode - not realtime] Publish framerate forced to SVO "
                "Playback rate");
    mVdPubRate = 0;
  } else {
...
void ZedCamera::getPosTrackingParams() {
  ...
  for (int idx = static_cast<int>(sl::POSITIONAL_TRACKING_MODE::GEN_1);
    ...
    DEBUG_PT(" Comparing '%s' to '%s'", test_mode_str.c_str(),
             pos_trk_mode_str.c_str());
...
void ZedCamera::initPublishers() {
  ...
  if (mSvoMode) {
    mPubSvoStatus = create_publisher<zed_msgs::msg::SvoStatus>(svo_status_topic,
                                                               mQos, mPubOpt);
    ...
    if (mUseSvoTimestamp && mPublishSvoClock) {
      ...
      RCLCPP_INFO_STREAM(
          get_logger(), "Advertised on topic: " << mPubClock->get_topic_name());
    }
  }
  // <---- SVO Status publisher
