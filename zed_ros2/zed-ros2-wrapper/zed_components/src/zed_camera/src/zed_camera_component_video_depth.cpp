...
void ZedCamera::getDepthParams() {
  ...
  if (!matched) {
    ...
    if (depth_mode_str != "NEURAL_LIGHT") {
      ...
      RCLCPP_WARN_STREAM(get_logger(), "Using default value: "
...
bool ZedCamera::areVideoDepthSubscribed() {
  ...
  try {
    ...
    rcutils_reset_error();
    DEBUG_STREAM_VD(
        " * [areVideoDepthSubscribed] Exception while counting subscribers");
    return false;
  }
...
void ZedCamera::applyAutoExposureGainSettings() {
  ...
  if (mTriggerAutoExpGain) {
    ...
    if (err != sl::ERROR_CODE::SUCCESS) {
      RCLCPP_WARN_STREAM(
          get_logger(), "Error setting AEC_AGC: " << sl::toString(err).c_str());
    } else {
...
void ZedCamera::applyExposureGainSettings() {
  ...
  if (!mCamAutoExpGain) {
    ...
    if (err != sl::ERROR_CODE::SUCCESS) {
      RCLCPP_WARN_STREAM(
          get_logger(),
          "Error setting " << sl::toString(sl::VIDEO_SETTINGS::EXPOSURE).c_str()
...
void ZedCamera::applyZEDXAutoExposureTimeRange() {
  ...
  if (err == sl::ERROR_CODE::SUCCESS &&
    ...
    err = mZed->setCameraSettings(sl::VIDEO_SETTINGS::AUTO_EXPOSURE_TIME_RANGE,
...
void ZedCamera::applyZEDXAutoAnalogGainRange() {
  ...
  if (err != sl::ERROR_CODE::SUCCESS) {
    RCLCPP_WARN_STREAM(
        get_logger(),
        "Error setting "
            << sl::toString(sl::VIDEO_SETTINGS::AUTO_ANALOG_GAIN_RANGE).c_str()
            << ": " << sl::toString(err).c_str());
  }
}
...
void ZedCamera::applyZEDXAutoDigitalGainRange() {
  ...
  if (err != sl::ERROR_CODE::SUCCESS) {
    RCLCPP_WARN_STREAM(
        get_logger(),
        "Error setting "
            << sl::toString(sl::VIDEO_SETTINGS::AUTO_DIGITAL_GAIN_RANGE).c_str()
            << ": " << sl::toString(err).c_str());
  }
}
...
bool ZedCamera::retrieveDepthMap() {
  if (mDepthSubCount > 0 || mDepthInfoSubCount > 0) {
    ...
    bool ok = sl::ERROR_CODE::SUCCESS ==
              mZed->retrieveMeasure(mMatDepth, sl::MEASURE::DEPTH, sl::MEM::CPU,
                                    mMatResol);
    if (ok) {
...
bool ZedCamera::checkGrabAndUpdateTimestamp(rclcpp::Time &out_pub_ts) {
  ...
  if (mSdkGrabTS.data_ns != 0) {
    if (!mSvoMode) {
      ...
      DEBUG_STREAM_VD(" * VIDEO/DEPTH PUB MEAN PERIOD: "
                      ...
                      << 1. / mVideoDepthPeriodMean_sec->getAvg()
                      << " Hz / Expected: " << 1. / mVdPubRate << " sec @"
                      << mVdPubRate << " Hz");
  ...
  if (mSvoMode) {
    ...
    out_pub_ts =
        sl_tools::slTime2Ros(mSdkGrabTS, get_clock()->get_clock_type());
  }
  return true;
}
...
void ZedCamera::publishStereoImages(const rclcpp::Time &t) {
  if (mStereoSubCount > 0) {
    ...
    auto combined =
        sl_tools::imagesToROSmsg(mMatLeft, mMatRight, mCameraFrameId, t);
...
void ZedCamera::publishStereoRawImages(const rclcpp::Time &t) {
  if (mStereoRawSubCount > 0) {
    ...
    auto combined =
        sl_tools::imagesToROSmsg(mMatLeftRaw, mMatRightRaw, mCameraFrameId, t);
...
void ZedCamera::publishImageWithInfo(sl::Mat &img,
  ...
  DEBUG_STREAM_VD(" * Publishing IMAGE message: " << t.nanoseconds()
                                                  << " nsec");
  try {
...
void ZedCamera::publishDepthMapWithInfo(sl::Mat &depth, rclcpp::Time t) {
  ...
  if (!mOpenniDepthMode) {
    ...
    DEBUG_STREAM_VD(" * Publishing DEPTH message: " << t.nanoseconds()
                                                    << " nsec");
    try {
...
void ZedCamera::processPointCloud() {
  ...
  if (isPointCloudSubscribed()) {
    ...
    if (pc_lock.try_lock()) {
      DEBUG_STREAM_PC(" * [processPointCloud] Retrieving point cloud size: "
      ...
      DEBUG_STREAM_PC(" * [processPointCloud] Retrieved point cloud size: "
                      << mMatCloud.getWidth() << "x" << mMatCloud.getHeight());
      ...
      DEBUG_STREAM_PC(" * [processPointCloud] Extracted point cloud: "
                      << mMatCloud.getInfos().c_str());
    } else {
...
void ZedCamera::publishPointCloud() {
  ...
  try {
    ...
    DEBUG_STREAM_PC(
        " * [publishPointCloud] Message publishing ecception: " << e.what());
  } catch (...) {
    DEBUG_STREAM_PC(
        " * [publishPointCloud] Message publishing generic ecception");
  }
#else
  try {
    mPubCloud->publish(std::move(pcMsg));
...
void ZedCamera::threadFunc_videoDepthElab() {
  ...
  while (1) {
    if (!rclcpp::ok()) {
      DEBUG_VD(
          " * [threadFunc_videoDepthElab] Ctrl+C received: stopping video "
          "depth thread");
      break;
    }
    if (mThreadStop) {
      DEBUG_VD(" * [threadFunc_videoDepthElab] Video/Depth thread stopped");
      break;
    }
...
void ZedCamera::setupVideoDepthThread() {
  ...
  if (_debugAdvanced) {
    ...
    if (pthread_getschedparam(pthread_self(), &policy, &par)) {
      ...
      DEBUG_STREAM_ADV(" * Default Video/Depth thread (#"
  ...
  if (_debugAdvanced) {
    ...
    if (pthread_getschedparam(pthread_self(), &policy, &par2)) {
      ...
      DEBUG_STREAM_ADV(" * New Video/Depth thread (#"
                       << pthread_self() << ") settings - Policy: "
                       << sl_tools::threadSched2Str(policy).c_str()
...
bool ZedCamera::waitForVideoDepthData(std::unique_lock<std::mutex> &lock) {
  while (!mVdDataReady) {  // loop to avoid spurious wakeups
    if (mVdDataReadyCondVar.wait_for(lock, std::chrono::milliseconds(500)) ==
      ...
      if (!rclcpp::ok()) {
        DEBUG_VD(
            "[waitForVideoDepthData] Ctrl+C received: stopping video/depth "
            "thread");
        mThreadStop = true;
        return false;
      }
      if (mThreadStop) {
        DEBUG_VD("[waitForVideoDepthData] Video/Depth thread stopped");
        return false;
      }
  ...
  DEBUG_STREAM_VD(" * [waitForVideoDepthData] mVdMutex: "
...
void ZedCamera::handleVideoDepthPublishing() {
  ...
  DEBUG_STREAM_VD(" * [handleVideoDepthPublishing] elapsed_usec "
                  << elapsed_usec);
  ...
  DEBUG_STREAM_VD(" * [handleVideoDepthPublishing] sleeped for " << wait_usec
                                                                 << " µsec");
...
bool ZedCamera::waitForPointCloudData(std::unique_lock<std::mutex> &lock) {
  while (!mPcDataReady) {  // loop to avoid spurious wakeups
    if (mPcDataReadyCondVar.wait_for(lock, std::chrono::milliseconds(500)) ==
      ...
      if (!rclcpp::ok()) {
        DEBUG_PC(
            " * [waitForPointCloudData] Ctrl+C received: stopping point cloud "
            "thread");
        mThreadStop = true;
        return false;
      }
      if (mThreadStop) {
        DEBUG_PC(" * [waitForPointCloudData] Point Cloud thread stopped");
        return false;
      }
  ...
  DEBUG_STREAM_PC(" * [waitForPointCloudData] mPcMutex: "
                  << (lock.owns_lock() ? "Locked" : "Unlocked"));
  return true;
}
...
void ZedCamera::handlePointCloudPublishing() {
  ...
  DEBUG_STREAM_PC(" * [handlePointCloudPublishing] elapsed_usec "
                  << elapsed_usec);
  ...
  DEBUG_STREAM_PC(" * [handlePointCloudPublishing] sleeped for " << wait_usec
                                                                 << " µsec");
...
void ZedCamera::threadFunc_pointcloudElab() {
  ...
  while (1) {
    if (!rclcpp::ok()) {
      DEBUG_PC(
          " * [threadFunc_pointcloudElab] Ctrl+C received: stopping point "
          "cloud thread");
      break;
    }
    if (mThreadStop) {
...
bool ZedCamera::handleVideoDepthDynamicParams(
  ...
  if (sl_tools::isZEDX(mCamRealModel)) {
    if (handleGmsl2Params(param, result)) {
      return true;
    }
  } else {
    if (handleUsb3Params(param, result)) {
      return true;
    }
  }
  if (handleCommonVideoParams(param, result)) {
    return true;
  }
  if (handleDepthParams(param, result)) {
    return true;
  }
...
bool ZedCamera::handleGmsl2Params(
  ...
  if (name == "video.exposure_time" ||
      ...
      name == "video.exposure_compensation" || name == "video.analog_gain" ||
    ...
    RCLCPP_INFO_STREAM(get_logger(),
                       "Parameter '" << name << "' correctly set to " << val);
    return true;
  }
  return false;
}
...
bool ZedCamera::handleUsb3Params(
    ...
    rcl_interfaces::msg::SetParametersResult &result) {
  const std::string &name = param.get_name();
  if (name == "video.brightness" || name == "video.contrast" ||
      name == "video.hue") {
    if (sl_tools::isZEDX(mCamRealModel)) {
      RCLCPP_WARN_STREAM(get_logger(),
                         "Parameter '" << name << "' not available for "
...
bool ZedCamera::handleCommonVideoParams(
    ...
    rcl_interfaces::msg::SetParametersResult &result) {
  const std::string &name = param.get_name();
  if (name == "video.saturation" || name == "video.sharpness" ||
      name == "video.gamma" || name == "video.exposure" ||
      name == "video.gain" || name == "video.whitebalance_temperature") {
    rclcpp::ParameterType correctType =
        rclcpp::ParameterType::PARAMETER_INTEGER;
    if (param.get_type() != correctType) {
      result.successful = false;
...
bool ZedCamera::handleDepthParams(
  ...
  if (name == "depth.point_cloud_freq") {
    ...
    RCLCPP_INFO_STREAM(get_logger(),
                       "Parameter '" << name << "' correctly set to " << val);
    return true;
  } else if (name == "depth.depth_confidence" ||
             name == "depth.depth_texture_conf") {
    rclcpp::ParameterType correctType =
        rclcpp::ParameterType::PARAMETER_INTEGER;
    if (param.get_type() != correctType) {
      result.successful = false;
