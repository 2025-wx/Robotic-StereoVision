...
void ZedCamera::getOdParams() {
  ...
  sl_tools::getParam(shared_from_this(), "object_detection.prediction_timeout",
                     ...
                     " * Object Det. prediction timeout [sec]: ", false, 0.0,
                     300.0);
  ...
  for (int idx = static_cast<int>(sl::OBJECT_FILTERING_MODE::NONE);
    ...
    std::replace(
        test_mode_str.begin(), test_mode_str.end(), ' ',
        '_');  // Replace spaces with underscores to match the YAML setting
    DEBUG_OD(" Comparing '%s' to '%s'", filtering_mode_str.c_str(),
             test_mode_str.c_str());
  ...
  if (!matched) {
    RCLCPP_WARN_STREAM(get_logger(),
                       ...
                       "'object_detection.filtering_mode' is not valid: '"
                           << filtering_mode_str
  ...
  if (mUsingCustomOd) {
    ...
    sl_tools::getParam(shared_from_this(),
                       ...
                       " * MultiClassBox animals confidence: ", true, 0.0,
                       100.0);
    sl_tools::getParam(
        shared_from_this(),
    ...
    sl_tools::getParam(
        ...
        " * MultiClassBox fruits and vegetables confidence: ", true, 0.0,
        100.0);
    sl_tools::getParam(
        shared_from_this(), "object_detection.class.sport.confidence_threshold",
        mObjDetSportConf, mObjDetSportConf,
        " * MultiClassBox sport-related objects confidence: ", true, 0.0,
        100.0);
    // <---- MultiClassBox parameters
  }
}
...
void ZedCamera::getCustomOdParams() {
  ...
  sl_tools::getParam(shared_from_this(),
                     ...
                     mYoloOnnxSize, " * Custom ONNX input size: ", false, 128,
                     2048);
  sl_tools::getParam(shared_from_this(), "object_detection.custom_class_count",
                     mCustomClassCount, mCustomClassCount,
                     " * Custom ONNX class count: ", false, 1, 999);
  ...
  for (int i = 0; i < mCustomClassCount; i++) {
    ...
    std::string param_prefix =
        std::string("object_detection.") + class_prefix.str() + ".";

    param_name = param_prefix + "label";
    sl_tools::getParam(shared_from_this(), param_name, label, label,
                       std::string("  * ") + param_name + ": ", false);

    param_name = param_prefix + "model_class_id";
    sl_tools::getParam(shared_from_this(), param_name, class_id, class_id,
                       std::string("  * ") + param_name + ": ", false, 0, 999);

    mCustomClassIdMap[class_prefix.str()] =
        class_id;  // Update the class prefix to class_id mapping
    ...
    sl_tools::getParam(shared_from_this(), param_name,
                       customOdProperties.enabled, customOdProperties.enabled,
                       std::string("  * ") + param_name + ": ", true);

    param_name = param_prefix + "confidence_threshold";
    sl_tools::getParam(shared_from_this(), param_name,
                       customOdProperties.detection_confidence_threshold,
                       customOdProperties.detection_confidence_threshold,
                       std::string("  * ") + param_name + ": ", true, 0.0f,
                       100.0f);
    param_name = param_prefix + "is_grounded";
    sl_tools::getParam(shared_from_this(), param_name,
                       customOdProperties.is_grounded,
                       customOdProperties.is_grounded,
                       std::string("  * ") + param_name + ": ", true);
    param_name = param_prefix + "is_static";
    sl_tools::getParam(shared_from_this(), param_name,
                       customOdProperties.is_static,
                       customOdProperties.is_static,
                       std::string("  * ") + param_name + ": ", true);
    param_name = param_prefix + "tracking_timeout";
    sl_tools::getParam(
        shared_from_this(), param_name, customOdProperties.tracking_timeout,
        customOdProperties.tracking_timeout,
        std::string("  * ") + param_name + ": ", true, -1.0f, 300.0f);
    param_name = param_prefix + "tracking_max_dist";
    sl_tools::getParam(
        shared_from_this(), param_name, customOdProperties.tracking_max_dist,
        customOdProperties.tracking_max_dist,
        std::string("  * ") + param_name + ": ", true, -1.0f, 100.0f);
    param_name = param_prefix + "max_box_width_normalized";
    sl_tools::getParam(shared_from_this(), param_name,
                       customOdProperties.max_box_width_normalized,
                       customOdProperties.max_box_width_normalized,
                       std::string("  * ") + param_name + ": ", true, -1.0f,
                       1.0f);
    param_name = param_prefix + "min_box_width_normalized";
    sl_tools::getParam(shared_from_this(), param_name,
                       customOdProperties.min_box_width_normalized,
                       customOdProperties.min_box_width_normalized,
                       std::string("  * ") + param_name + ": ", true, -1.0f,
                       1.0f);
    ...
    sl_tools::getParam(
        ...
        std::string("  * ") + param_name + ": ", true, -1.0f, 10000.0f);
    param_name = param_prefix + "min_box_width_meters";
    sl_tools::getParam(
        shared_from_this(), param_name, customOdProperties.min_box_width_meters,
        customOdProperties.min_box_width_meters,
        std::string("  * ") + param_name + ": ", true, -1.0f, 10000.0f);
    param_name = param_prefix + "max_box_height_meters";
    sl_tools::getParam(shared_from_this(), param_name,
                       customOdProperties.max_box_height_meters,
                       customOdProperties.max_box_height_meters,
                       std::string("  * ") + param_name + ": ", true, -1.0f,
                       10000.0f);
    param_name = param_prefix + "max_allowed_acceleration";
    sl_tools::getParam(shared_from_this(), param_name,
                       customOdProperties.max_allowed_acceleration,
                       customOdProperties.max_allowed_acceleration,
                       std::string("  * ") + param_name + ": ", true, 0.0f,
                       100000.0f);
    ...
    sl_tools::getParam(shared_from_this(), param_name, acc_preset_str,
                       acc_preset_str);
    ...
    for (int idx = static_cast<int>(sl::OBJECT_ACCELERATION_PRESET::DEFAULT);
      ...
      std::replace(
          test_mode_str.begin(), test_mode_str.end(), ' ',
          '_');  // Replace spaces with underscores to match the YAML setting
      DEBUG_OD(" Comparing '%s' to '%s'", acc_preset_str.c_str(),
               test_mode_str.c_str());
    ...
    RCLCPP_INFO_STREAM(
        get_logger(),
        std::string("  * ") + param_name + ": "
            << sl::toString(customOdProperties.object_acceleration_preset)
                   .c_str());

    mCustomOdProperties[class_id] =
        customOdProperties;  // Update the Custom OD Properties information
  }
}
...
bool ZedCamera::handleOdDynamicParams(
  ...
  } else if (param.get_name() == "object_detection.class.electronics.enabled") {
    rclcpp::ParameterType correctType = rclcpp::ParameterType::PARAMETER_BOOL;
    if (param.get_type() != correctType) {
      result.successful = false;
      result.reason =
...
bool ZedCamera::handleCustomOdDynamicParams(
  ...
  DEBUG_STREAM_COMM(
      "handleCustomOdDynamicParams: Parameter name: " << param_full_name);

  if (param_full_name.find("object_detection.class_") == std::string::npos) {
    DEBUG_STREAM_COMM("handleCustomOdDynamicParams: Parameter '"
                      << param_full_name
  ...
  std::string class_id_str =
      param_full_name.substr(param_full_name.find("class_"), 9);
  ...
  if (mCustomClassIdMap.find(class_id_str) == mCustomClassIdMap.end()) {
    DEBUG_STREAM_COMM("handleCustomOdDynamicParams: Class ID '"
                      << class_id_str
  ...
  std::string param_name =
      param_full_name.substr(param_full_name.find_last_of('.') + 1);
  DEBUG_STREAM_COMM(
      "handleCustomOdDynamicParams: Custom OD Parameter name: " << param_name);
  ...
  if (param_name == "enabled") {
    ...
    RCLCPP_INFO_STREAM(
        get_logger(), "Parameter '"
                          << param.get_name() << "' correctly set to "
                          << (mCustomOdProperties[class_id].enabled ? "TRUE"
                                                                    : "FALSE"));
    ...
    RCLCPP_INFO_STREAM(
        ...
        "Parameter '" << param.get_name() << "' correctly set to "
                      << (mCustomOdProperties[class_id].is_grounded ? "TRUE"
                                                                    : "FALSE"));
    ...
    RCLCPP_INFO_STREAM(
        get_logger(),
        "Parameter '" << param.get_name() << "' correctly set to "
                      << (mCustomOdProperties[class_id].is_static ? "TRUE"
                                                                  : "FALSE"));
...
void ZedCamera::processDetectedObjects(rclcpp::Time t) {
  ...
  if (!mUsingCustomOd || ZED_SDK_MAJOR_VERSION < 5) {
    ...
    if (mObjDetElectronicsEnable) {
      ...
      mObjDetClassConfMap[sl::OBJECT_CLASS::ELECTRONICS] =
          mObjDetElectronicsConf;
    }
    ...
    objectTracker_parameters_rt.object_class_detection_confidence_threshold =
        mObjDetClassConfMap;
    // <---- Process realtime dynamic parameters

    objDetRes = mZed->retrieveObjects(objects, objectTracker_parameters_rt,
                                      mObjDetInstID);
  }
    ...
    sl::CustomObjectDetectionRuntimeParameters
        custom_objectTracker_parameters_rt;
    custom_objectTracker_parameters_rt.object_class_detection_properties =
        mCustomOdProperties;  // Update realtime detection parameters
    // <---- Process realtime dynamic parameters
  ...
  for (auto data : objects.object_list) {
    ...
    DEBUG_STREAM_OD(" * Object ID:"
                    << data.id << " - " << objMsg->objects[idx].label << " ["
                    << objMsg->objects[idx].sublabel
                    << "] - Confidence: " << objMsg->objects[idx].confidence);
