 ...
 */
template <typename T>
void getParam(const std::shared_ptr<rclcpp::Node> node, std::string paramName,
              T defValue, T &outVal, std::string log_info = std::string(),
              bool dynamic = false, T minVal = std::numeric_limits<T>::min(),
 ...
 * @param out_labels the map containing the labels. The map idx corresponds to
 * the class ID
 * @return true if successfull
 */
bool ReadCocoYaml(const std::string &label_file,
...
class StopWatch {
 public:
  ...
  double toc(std::string func_name =
                 std::string());  //!< Returns the seconds elapsed from the last
                                  //!< tic in ROS clock reference (it works also
                                  //!< in simulation)
...
template <typename T>
void getParam(const std::shared_ptr<rclcpp::Node> node, std::string paramName,
              T defValue, T &outVal, std::string log_info, bool dynamic,
              T minVal, T maxVal) {
  ...
  if constexpr (std::is_same<T, double>::value) {
    descriptor.additional_constraints = "Range: [" + std::to_string(minVal) +
  ...
  node->declare_parameter(paramName, rclcpp::ParameterValue(defValue),
                          descriptor);
