...
void initCostTraversibily(sl::Terrain &cost_terrain, float resolution,
                          float range, float height_threshold) {
...
void computeCost(sl::Terrain &elevation_terrain, sl::Terrain &cost_terrain,
  ...
  auto factor_step_ =
      traversability_parameters.step_weight / robot_parameters.step_max;
  auto factor_slope_ =
      traversability_parameters.slope_weight / robot_parameters.slope_max;
  ...
  int nb_cells =
      (2. * a_rad) /
      reso_d;  // big agent with small grid size is heavier to compute
  ...
  for (auto chunk_id : chunks_idx) {
    ...
    auto &cost_step_data =
        chunk_cost.getLayer(TRAVERSABILITY_COST_STEP).getData();
    auto &cost_slope_data =
        chunk_cost.getLayer(TRAVERSABILITY_COST_SLOPE).getData();
    auto &cost_roughness_data =
        chunk_cost.getLayer(TRAVERSABILITY_COST_ROUGHNESS).getData();
    ...
    for (unsigned int idx_current = 0; idx_current < size_; idx_current++) {
      ...
      if (std::isfinite(ref_height)) {
        ...
        for (int x_ = 0; x_ < nb_cells; x_++) {
          ...
          for (int y_ = 0; y_ < nb_cells; y_++) {
            ...
            if (dim.getIndex(x_v, y_v, idx_tmp) /*True = error*/) {
              ...
              curr_height = elevation_terrain.readValue(
                  sl::LayerName::ELEVATION, x_v, y_v);
            } else {
            ...
            if (std::isfinite(curr_height)) {
              ...
              max_diff_height =
                  std::max(max_diff_height, fabsf32(curr_height - ref_height));
            }
          }
        }
        ...
        cost = clamp((roughness * factor_roughness_ + slope * factor_slope_ +
                      step * factor_step_) *
                         0.3f,
                     0.f, 1.f);
        cost_data[idx_current] = cost;
        occupancy_data[idx_current] =
            (cost > traversability_parameters.occupancy_threshold)
                ? OCCUPIED_CELL
                : FREE_CELL;
        if (slope == 0) {
...
void normalization(sl::Terrain &cost_terrain, sl::LayerName layer,
                   sl::Mat &view) {
  sl::Mat cost;
