...
from robotic_driver.robot_state.joint_state_handler import JointStateHandler
from robotic_driver.robot_state.robot_state_handler import RobotStateHandler
from robotic_driver.robot_state.io_state_handler import IOStateHandler
from robotic_driver.robot_state.gripper_state_handler import GripperStateHandler
...
class RobotStateInterface(Node):
    def __init__(self):
        ...
        self.joint_state_handler_ = JointStateHandler(self, self.robot_, self.joints_name_, self.gripper_name_)
        self.robot_state_handler_ = RobotStateHandler(self, self.robot_)
        self.io_state_handler_ = IOStateHandler(self, self.robot_)
        if self.has_gripper_:
            self.gripper_state_handler_ = GripperStateHandler(self, self.robot_)
