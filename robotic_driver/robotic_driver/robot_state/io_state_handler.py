...
class IOStateHandler:
    ...
    def timer_callback(self):
        ...
        for i in range(4):
            io_status.robot_din.append(bool(self.robot_.get_di(i)))
        ain_type_data = list()
        for i in range(2):
            io_status.robot_ain.append(self.robot_.get_ai(i))
            ain_type_data.append(self.robot_.get_ai_mode(i))
        io_status.robot_ain_type = ain_type_data
        # flange_di_data = list()
        for i in range(2):
            io_status.flange_din.append(bool(self.robot_.get_flange_di(i)))
        self.io_state_pub_.publish(io_status)
        for i in range(2):
            io_status.extend_din.append(bool(self.robot_.get_extra_di(i)))
            io_status.extend_ain.append(self.robot_.get_extra_ai(i))
        self.io_state_pub_.publish(io_status)
