...
class JointStateHandler:
    def __init__(self, node: Node, robot: LebaiRobot, joints_name, gripper_name):
        self.node_ = node
        self.robot_ = robot
    ...
    def timer_callback(self):
        if rclpy.ok():
            ...
            positions = self.robot_.get_actual_joint_positions()
            velocities = self.robot_.get_actual_joint_speeds()
            torques = self.robot_.get_actual_joint_torques()
