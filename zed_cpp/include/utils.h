...
inline cv::Scalar generateColorID_u(int idx) {
  ...
  return cv::Scalar(CLASS_COLORS[color_idx][0], CLASS_COLORS[color_idx][1],
                    CLASS_COLORS[color_idx][2], 255);
}

inline sl::float4 generateColorID_f(int idx) {
  auto clr_u = generateColorID_u(idx);
  return sl::float4(static_cast<float>(clr_u.val[0]) / 255.f,
                    static_cast<float>(clr_u.val[1]) / 255.f,
                    static_cast<float>(clr_u.val[2]) / 255.f, 1.f);
}
...
inline bool renderObject(const sl::ObjectData &i, const bool isTrackingON) {
  if (isTrackingON)
    ...
    return (i.tracking_state == sl::OBJECT_TRACKING_STATE::OK ||
            i.tracking_state == sl::OBJECT_TRACKING_STATE::OFF);
}
...
inline sl::float4 getColorClass(int idx) {
  idx = std::min(5, idx);
  sl::float4 clr(class_colors[idx][0], class_colors[idx][1],
                 class_colors[idx][2], 1.f);
  return clr / 255.f;
}

template <typename T>
inline uchar _applyFading(T val, float current_alpha, double current_clr) {
  return static_cast<uchar>(current_alpha * current_clr +
                            (1.0 - current_alpha) * val);
}

inline cv::Vec4b applyFading(cv::Scalar val, float current_alpha,
...
inline void drawVerticalLine(cv::Mat &left_display, cv::Point2i start_pt,
                             cv::Point2i end_pt, cv::Scalar clr,
                             int thickness) {
  int n_steps = 7;
  cv::Point2i pt1, pt4;
